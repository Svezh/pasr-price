...
def PlotBilder(ref):
    ...
    mask=df_comp.duplicated(subset=['Model','Date'])
    ...
    df_check=pd.DataFrame(df_comp_cit.groupby(by=['Time','Date']).groups)
    a=(df_check.columns)
    checker = []
    ...
    for i in range(int(len(checker))):
        if checker[i] > max:
            ...
            checker_max.append(checker[i])
#Датафрейм для нахождения индексов итогового фрейма

    #df_comp_cit добавить нужные индексы
    ...
    df_ref_xcm= df_ref.loc[(df_ref['ART_Model'].isin(df_comp['Model'])) | (df_ref['Name_Model'].isin(df_comp['Model'])) ]
    df_ref_xcm.reset_index(drop=True, inplace=True)
    df_ref_cit= df_ref.loc[(df_ref['ART_Model'].isin(df_comp_cit['Model'])) | (df_ref['Name_Model'].isin(df_comp_cit['Model'])) ]
...
###Проверка на заполненность датафрема проверки
    if df_comp_cit.empty:
        ...
        for i in range(int(len(df_comp_cit['Current_Price']))):
            for j in range(int(len(df_ref_cit))):
                if (df_comp_cit['Model'][i] == df_ref_cit['ART_Model'][j]) or (df_comp_cit['Model'][i] == df_ref_cit['Name_Model'][j]):
        ...
        for j in range (int(len(df_ref_cit['ART_Model']))):
            ...
            data_x=df_comp_cit['Time'].loc[(df_comp_cit['Model'] == df_ref_cit['Name_Model'][j]) | (df_comp_cit['Model'] == df_ref_cit['ART_Model'][j])]
            data_y=df_comp_cit['Current_Price'].loc[(df_comp_cit['Model'] == df_ref_cit['Name_Model'][j]) | (df_comp_cit['Model'] == df_ref_cit['ART_Model'][j])]
    ...
    if df_comp.empty:
        ...
        for i in range(int(len(df_ref_xcm['ART_Model']))):
            ...
            df_comp['Current_Price'].loc[df_comp['Model']==df_ref_xcm['ART_Model'][i]], '.-', label= label,linewidth=2)
